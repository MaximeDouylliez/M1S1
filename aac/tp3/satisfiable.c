...
/* ##################################################################Fonctions d'affichage +init cert########################################################################*/
...
struct clause * clauseNLit_to_clause(struct clauseNLit nc,struct formuleNLit* fl){
  ...
  struct clause* c;
  struct litteral ld;
  if(nc.nlit==1){c=malloc(sizeof (struct clause));c[0].l1=nc.litt[0];c[0].l2=nc.litt[0];c[0].l3=nc.litt[0];}
  else if(nc.nlit==2){c=malloc(sizeof (struct clause));c[0].l1=c[0].l2=nc.litt[0];c[0].l3=nc.litt[1];}
  else if(nc.nlit==3){c=malloc(sizeof (struct clause));c[0].l1=nc.litt[0];c[0].l2=nc.litt[1];c[0].l3=nc.litt[2];}
  else {
    ...
    c[nc.nlit-1].l3=nc.litt[nc.nlit-1];
...
struct formule formule_sat_to_formule_3sat(struct formuleNLit fnl){
  ...
  for (i_fnl=0;i_fnl<fnl.nclause;i_fnl++){
    clausenl=fnl.tclause[i_fnl];
    if(clausenl.nlit<=3){bufferc=clauseNLit_to_clause(clausenl,&fnl);f.tclause[cpt_clause]=bufferc[0];cpt_clause++;}
    else {bufferc=clauseNLit_to_clause(clausenl,&fnl);for(j=0;j<clausenl.nlit;j++){f.tclause[cpt_clause]=bufferc[j];cpt_clause++;}}
...
printf("\n\n l'algo va traiter %d clause\n\n",f.nclause);
  for(i=0;i<f.nclause && sat==1;i++){
	...
	else if ((i==f.nclause-1)|| (!temp.is_red) ||(temp.seq_red!=c.seq_red))
	  {
	...
	else{index_join_var++;// ind_seq_red++;
	  if(!(lit_val(c.l1,cert)||lit_val(c.l2,cert)))
	    {
...
int main(int arc,char**argv){
  ...
  struct certificat c;
   ...
   if (atoi(argv[1])==0) //les trois options ci dessous utilise le fichier texte pour créer la formule
